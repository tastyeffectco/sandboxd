...
export function StoreView({ onError, toast, onOpen, reloadApps }: { onError: (m: string) => void; toast: (m: string) => void; onOpen: (id: string) => void; reloadApps: () => void }) {
  ...
  const install = async (r: CatalogRecipe) => {
    ...
    try {
      ...
      await api.putWorkspaceFile(sb.id, 'catalog-run.sh', r.script)
      await api.putWorkspaceFile(sb.id, 'sandbox.yaml', recipeManifest(r))
      await api.putWorkspaceFile(sb.id, 'AGENTS.md', recipeAgentsMd(r))
