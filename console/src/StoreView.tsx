        ...
        {items.map((r) => (
          <Card key={r.id} style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ width: 30, height: 30, borderRadius: 8, background: c.panel2, border: `1px solid ${c.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', ...{ fontFamily: font.mono }, fontSize: 11, fontWeight: 600, color: c.muted }}>{initials(r.name)}</span>
